...
Matamikya matamikyaCreate()
{
    ...
    new_warehouse->storage = asCreate(copyProduct, freeProduct, compareProduct); 
    if(new_warehouse->storage == NULL){
        matamikyaDestroy(new_warehouse);  
        return NULL;
    }
    new_warehouse->orders = setCreate(copyOrder, freeOrder, compareOrder);  
...
{   
    ...
    double current_amount_in_storage = 0;
    asGetAmount(matamikya->storage, (ASElement)ptr, &current_amount_in_storage);
    current_amount_in_storage += amount;
    if (checkProductAmountValid(current_amount_in_storage, amount_type) != MATAMIKYA_SUCCESS) {
        return MATAMIKYA_INVALID_AMOUNT;
    }
                  ...
                  const unsigned int productId, const double amount)
{
    ...
    asGetAmount(ptr_order, (ASElement)product, &current_amount_in_order);
    current_amount_in_order += amount;
    if (checkProductAmountValid(current_amount_in_order, product_amount_type) != MATAMIKYA_SUCCESS) {
        return MATAMIKYA_INVALID_AMOUNT;
    }
    if(checkIfItemExistInOrderById(ptr_order, productId) == false){
        ...
        freeSingleleItemInOrder(new_product_id); 
    }
